            ...
            <div class="grid grid-cols-1 text-center items-center justify-items-center min-h-screen">
                <div class="w-3/4 p-4">
                    ...
                    <div :class="{'opacity-0 invisible h-0': intro !== 3, 'opacity-1 visible': intro === 3}" class="transition-all">
                        ...
                        <div class="fixed bottom-0 left-0 p-6 w-full">
                            <button :disabled="user.name === null" @click="intro += 1" :class="{'shadow-indigo-800': user.name }" class="btn btn-primary w-full rounded-xl shadow-[0px_2px_0px_3px]">Lanjutkan</button>
                        </div>
                    </div>
                </div>
            </div>
