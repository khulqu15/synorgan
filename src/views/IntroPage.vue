<template>
    <ion-page>
        <ion-content :padding-start="0" :padding-end="0" :padding-top="0" :padding-bottom="0" fullscreen>
            <div class="grid grid-cols-1 text-center min-h-screen bg-base-200 justify-items-center" :class="{'items-center': intro < 4, 'pt-12': intro >= 4}">
                ...
                <div class="p-4" :class="{'w-3/4': intro < 3, 'w-full': intro >= 3}">
                    ...
                    <div :class="{'opacity-0 invisible h-0 hidden': intro !== 4, 'opacity-1 visible': intro === 4}" class="transition-all">
                        ...
                        <div class="fixed bottom-0 z-10 left-0 p-6 w-full">
                            <button @click="user.class.type !== null ? saveData() : intro = intro" 
                                class="btn w-full rounded-xl shadow-[0px_2px_0px_3px]"
                                :class="{'btn-primary shadow-indigo-800': user.class.type !== null, 'shadow-gray-800': user.class.type === null}">Lanjutkan</button>
                        </div>
                    </div>
                </div>
            </div>
